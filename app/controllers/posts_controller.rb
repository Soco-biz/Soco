class PostsController < ApplicationController
  ...
  def create
    ...
    if @start_latitude < @user_latitude && @user_latitude < @end_latitude && @start_longitude < @user_longitude && @user_longitude  < @end_longitude || @room_id == "0" || @room_id == Rails.cache.read('lock_room')
      ...
      post = Post.new(content:params[:content], room:params[:roomId], image:@image_link,
        similarity: @mostSimId, simvalue: @mostSimvValue, latitude: params[:latitude], longitude: params[:longitude])
      post.save
    end
  end
    def similarity(post)
